...
SNR_{}_Tau_{}_set_{}.npz'.format(trial,
    ...
    #        print(i)
            if i==0:
                if testmode==1:
                    res_SVM=copy.deepcopy(result_set[0])
                else:
                    res_CNN=copy.deepcopy(result_CNN_set[0])
                    res_CNN_MB=copy.deepcopy(result_CNN_MB_set[0])
            
            else:
    #        for item in res_SVM.keys():
    #            res_SVM[item]+=copy.deepcopy(result_set[i][item])
                if testmode==1:
                    for item in res_SVM.keys():
                        res_SVM[item]+=copy.deepcopy(result_set[i][item])
                else:
        ...
        if testmode==1:
            for item in res_SVM.keys():
                res_SVM[item]=copy.deepcopy(res_SVM[item]/trial)
        else:
            for item in res_CNN.keys():
        ...
        if testmode==1:
            a,b,c,d,e=plot_figure_mse(res_SVM,3,4)
            a2,b2,c2,d2,e2=plot_figure_mse(res_SVM,3,4)
        else:
            a,b,c,d,e,f,g,h,i,j=plot_figure(res_CNN,3,4)
            a2,b2,c2,d2,e2,f2,g2,h2,i2,j2=plot_figure(res_CNN_MB,5,6)
#        a,b,c,d,e=plot_figure_mse(res_CNN,3,4)
#        a2,b2,c2,d2,e2=plot_figure_mse(res_CNN_MB,5,6)
            # BGD_cov[:,m]=find_conver(res_CNN,thres)
            # MBGD_cov[:,m]=find_conver(res_CNN_MB,thres)
    
        
        
        
        am=np.mean(SCA_Gibbs,axis=1)
        BGD_acc[0,m]=a
        BGD_acc[1,m]=b
        BGD_acc[2,m]=c
        BGD_acc[3,m]=d
        ...
        MBGD_acc[4,m]=e2
        OBJ_acc[0,m]=am[0]
        OBJ_acc[1,m]=am[len(am)-1]
        OBJ_acc[2,m]=np.mean(DC_NORIS_set)
        OBJ_acc[3,m]=np.mean(DC_NODS_set)
        OBJ_acc[4,m]=np.mean(DG_NORIS) 
