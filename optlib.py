    ...
    for it in range(libopt.nit):
        ...
        for i in range(I):
            ...
            if RISON:
                ...
                c[:, i] = np.abs(np.conjugate(f) @ h[:, i]) ** 2 + 2 * tau * K2[i] * (L + 1) + 2 * np.real(
...
def find_obj_inner(libopt, x, K, K2, Ksum2, h_d, G, f0, theta0, RISON):
    # compute the objective value for a given x
    N = libopt.N
    L = libopt.L
    M = libopt.M
    if sum(x) == 0:
        ...
        index = (x == 1)
        # optimize f and theta via Algorithm 1
...
def Gibbs(libopt, h_d, G, x0, RISON, Joint):
    ...
    Ksum2 = sum(K) ** 2
    x = x0
    # the following stores the retrun values to be compute
    ...
    beta = min(1, obj_new[ind])  # temperature parameter \beta
    alpha = 0.9  # cooling parameter \rho
    ...
    # store f and theta from the last iteration for warm-start in this iteration
    ...
    for j in range(Jmax):
        ...
        # 2--M+1-th trnasition, change only 1 entry
        for m in range(M):
            if libopt.verbose > 1:
            ...
            Temp[m + 1], _, f_loop[m + 1], theta_loop[m + 1] = find_obj_inner(libopt,
                                                                              x_sam, K, K2, Ksum2, h_d, G,
                                                                              f_loop[m + 1], theta_loop[m + 1], RISON)
            if libopt.verbose > 1:
